...
public class CTakeConter {
    ...
    //进入查看测试患者1页面
    @RequestMapping("ceshiahuan")
    public Object ceshiahaun(){

        return "cao/ceshiachuanzhe";
    }
    //查看患者信息
    @RequestMapping("selceshia")
    @ResponseBody
    public Object selceshia(ReportVo reportVo,Integer page, Integer limit){
        PageHelper.startPage(page, limit);
        List<ReportVo> selceshia = cTakeService.selceshia(reportVo);
        PageInfo pageInfo = new PageInfo(selceshia);
        Map<String, Object> tableData = new HashMap<String, Object>();
        //这是layui要求返回的json数据格式，如果后台没有加上这句话的话需要在前台页面手动设置
        tableData.put("code", 0);
        tableData.put("msg", "");
    ...
    //进入查看测试患者2页面
    @RequestMapping("ceshibhuan")
    public Object ceshibhaun(){

        return "cao/ceshibchuanzhe";
    }
    //查看患者信息
    @RequestMapping("selceshib")
    @ResponseBody
    public Object selceshib(ReportVo reportVo,Integer page, Integer limit){
        PageHelper.startPage(page, limit);
        List<ReportVo> selceshib = cTakeService.selceshib(reportVo);
        PageInfo pageInfo = new PageInfo(selceshib);
        Map<String, Object> tableData = new HashMap<String, Object>();
        //这是layui要求返回的json数据格式，如果后台没有加上这句话的话需要在前台页面手动设置
        tableData.put("code", 0);
        tableData.put("msg", "");
