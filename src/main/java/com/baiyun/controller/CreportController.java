...
@RequestMapping("cao")
    ...
    //查询所有数据
    @RequestMapping("ceshiaindex")
    public Object ceshiaoreport(ReportVo reportVo, Model model, String params, Integer cc, HttpServletRequest request){
        reportVo.setCc(cc);
        HttpSession session = request.getSession();
        //将数据存储到session中
        session.setAttribute("ban",cc);
        ...
        String reportName=params;
        reportVo.setReportName(reportName);
        List<ReportVo> sel = creportService.sel1(reportVo);
        model.addAttribute("report",sel);
        return "cao/ceshiareport";
    }
    //查询所有数据
    @RequestMapping("ceshibindex")
    public Object ceshibreport(ReportVo reportVo, Model model, String params, Integer cc, HttpServletRequest request){
        reportVo.setCc(cc);
        HttpSession session = request.getSession();
        //将数据存储到session中
        session.setAttribute("ban",cc);
        ...
        String reportName=params;
        reportVo.setReportName(reportName);
        List<ReportVo> sel = creportService.sel2(reportVo);
        model.addAttribute("report",sel);
        return "cao/ceshibreport";
    }
}
